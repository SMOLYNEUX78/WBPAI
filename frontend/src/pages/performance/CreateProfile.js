...
const CreateProfile = () => {
  ...
  const scanForSmartMeter = () => {
    setTimeout(() => {
      setProfile((prevProfile) => ({ ...prevProfile, smartMeterDetected: true }));
  ...
  const scanForSensors = () => {
    setTimeout(() => {
      setProfile((prevProfile) => ({ ...prevProfile, sensorDetected: true }));
      alert("Sensors detected and linked to Performance section!");
    }, 2000);
  };

  const handleCreateProfile = () => {
    // Process or validate profile data as needed.
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6">
      <div className="w-full max-w-3xl bg-white p-6 shadow-lg rounded-xl">
        ...
        <select
          ...
          value={profile.connectionMethod}
          onChange={handleChange}
